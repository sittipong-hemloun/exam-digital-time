...
describe("useExamInfo", () => {
  it("should initialize with empty exam info", () => {
    ...
    expect(result.current.examInfo.course).toBe("");
  ...
  it("should update form data when handleInputChange is called", () => {
    ...
    act(() => {
      result.current.handleInputChange("course", "CS101");
      result.current.handleInputChange("lecture", "01");
    });

    expect(result.current.formData.course).toBe("CS101");
  ...
  it("should update formData independently from examInfo", () => {
    ...
    expect(result.current.formData.course).toBe("CS101");
    expect(result.current.examInfo.course).toBe(""); // Still empty until confirmed
  });
  ...
  it("should allow multiple sequential changes", () => {
    ...
    act(() => {
      result.current.handleInputChange("course", "CS102");
    });

    expect(result.current.formData.course).toBe("CS102");

    act(() => {
      result.current.handleInputChange("course", "CS103");
    });

    expect(result.current.formData.course).toBe("CS103");
  });
});
